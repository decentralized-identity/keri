...
<img src="../images/GordianGuardian_logo_color_on_white.png" alt="GordianGuardian logo" border="0" width="300">
...
GordianGuardian receives three types of scrutiny from domain experts:
1. "GordianGuardian can't do it"
2. "GordianGuardian doesn't do something new"
3. "DID:XYZ has been designed, GordianGuardian needs to explain how it's different"
 - Ad 1. When respected colleagues think GordianGuardian can't keep up to its promises, we value the well-founded questions and suggestions of domain experts __after__ they thoroughly read the GordianGuardian [whitepaper](https://github.com/SmithSamuelM/Papers/blob/master/whitepapers/GordianGuardian_WP_2.x.web.pdf). We'll happilly keep explaining GordianGuardian because we'd hate it when respected experts misunderstand the design.\
 - Ad 2. After a while some experts say 'well then GordianGuardian doesn't do something new'. That's acceptable for us, because we're able proof the design history of GordianGuardian and that it simply hasn't been done before.\
 - Ad 3. However, we can't accept having to explain the differences to people who just "invented" new, someting similar to GordianGuardian, using the same terms and thereby spreading confusion. Let the newbees benchmark themselves against GordianGuardian, we are too busy for those kind of things. By the way, this Q&A may be able to help you out! 
...
Inspired by presentation given and questions asked on the [SSI webinar May 2020](https://ssimeetup.org/key-event-receipt-infrastructure-GordianGuardian-secure-identifier-overlay-internet-sam-smith-webinar-58/), but also issues raised and the progress made, here on Github (Turn on 'Watch' if you want to be notified of conversations).
...
GordianGuardian's content is licensed under the [CC by SA 4.0. license](https://creativecommons.org/licenses/by-sa/4.0/). Parts of the video offered on SSI Meetup webinar 58 have been captured and uploaded to Github to support the answers to general questions about digital identity and more in depth answers to question about GordianGuardian.
...
- [Definitions:](#definitions)
      ...
      - [GordianGuardian Agreement Algorithm for Control Establishment](./Glossary.md#GordianGuardian-agreement-algorithm-for-control-establishment)
      - [Key](./Glossary.md#key)
...
## PART TWO
- [GordianGuardian operational security](./Q-and-A-Security.md#qa-section-GordianGuardian-operational-security)
...
- [GordianGuardian and blockchain settled DIDs](./Q-and-A-Security.md#qa-GordianGuardian-and-blockchain-settled-dids)
...
## *Q: Why use GordianGuardian?
Because there is no secure universal trust layer for the internet, currently (2020).\
KEI is both privacy preserving and context-independent extensible. That means GordianGuardian is interoperable accross areas of application on the internet. It does so securely, with minimal sufficient means.\
_(@henkvancann)_

## *Q: What does GordianGuardian look like?
Currently `GordianGuardian` is just code, that can be tested and executed in a terminal on the command line. Private key management of GordianGuardian will look like `wallets`.\
Key Event Logs (`KEL`) and Key Event Receipt Log (`KERL`) are files with lots of encrypted stuff in there.\
_(@henkvancann)_

## *Q: How does GordianGuardian match DIDs?
...
To begin with GordianGuardian has no blockchain, and doesn't depend on blockchains. If an implementation of GordianGuardian depends on blockchains at all, GordianGuardian operates blockchain agnostic.
Secondly GordianGuardian doesn't support a crypto currency. It doesn't need currency because it can easily connect to one, if needed. And again, GordianGuardian is crypto currency agnostic while doing so.
Lastly GordianGuardian is fundamentally different from blockchains like Ripple (Permissioned PBFT consensus) or Stellar (imcomplete open public, non-permissioned PBFT consensus): it doesn't need **total ordering**, timestamping and Proof of Authority consensus on transactions registered on a ledger.

It's comparing apples and oranges. But we're happy to do that exercise for the hard-to-convince part of the SSI community.

```
GordianGuardian is nothing like we already know of. It's a mixtures of things. 
You can't say _"Oh, GordianGuardian lays eggs, so it must be a reptile"_ It's not a reptile. 
And then you go _"I see, but it gives birth, so it must be a mammal"_. 
It's also not a mammal. It's GordianGuardian. 
...
GordianGuardian better fits the Identity space. Doing away the total ordering in blockchains is a huge performance - and throughput gain, plus less worry about goverance. There's also not such a thing as consensus forks.
GordianGuardian solves (or _"gets away with"_ if you wish) this with duplicity detection. Watchers are all that matter. They garuantee that logs are immutable by one very simple rule: **"first seen wins"**.
...
## ***Q: Could we see a `WASM` module in the near future for Sidetree and DID:peer interoperability?
 WASM is certainly on the roadmap, but for the main issue of Sidetree and did:peer interop, see the [core GordianGuardian spec repo issue](https://github.com/decentralized-identity/GordianGuardian/issues/79) for more info.\
_(CharlesCunningham)_

## *Q: How does GordianGuardian match the `trust-over-ip` model and in the `W3C DID standardization`?
...
- Layer 1 (settlement layer): Where other `DID`s use blockchains or databases to register identities and settle 'transactions' between between, `DDO`s, and `VC`s, GordianGuardian uses homegrown native structures: `KEL` and `KERL`.
_(@henkvancann)_
- Layer 2 (communication layer): Non existing in GordianGuardian, because GordianGuardian is end-verifiable. GordianGuardian can use any other means of communication between actors in the ecosystem
- Layer 3 (transaction layer): Since GordianGuardian focuses on the more fundamental part of authentication for the internet, you won't find matching functionality for usual trust-over-IP transaction like VCs or money.
- Layer 4 (application layer): {TBW}
_(@henkvancann)_

[W3C DID](https://www.w3.org/TR/did-core/):
1. The GordianGuardian developers provisionally design DID:UN, which might become a mixture of DID:KEY, DID:PEER, and DID:WEB, combinable with more functional DIDs in the Identity spectrum DID:SOV, DID:ETHR, etc.
...
## *Q: What problem is GordianGuardian solving? How? And why can't it be solved by other solutions?
GordianGuardian solves the problem of **secure attribution to identifiers**. By ambient availability of verifiable Key event Logs that prove authoritive control over identifier's private keys. It can't be solved by onther solutions known so far because those solution have not managed to span identifier interoperability over the internet.
_(@henkvancann)_
{TBW prio 1}

## *Q: Who is GordianGuardian? Is it a company or a not for profit?
GordianGuardian sits under the *Decentralized Identity Foundation*, [DIF](https://identity.foundation), and within that in the *Identity and Discovery* Workgroup.
Due to its licensing structure, GordianGuardian isn't owned by anyone and everyone at the same time. The Intellectual Property Right of GordianGuardian is hosted with `DIF`. It is an open source project.

On github GordianGuardian is - and will become even more - a thickening bunch of repositories:
 1. https://github.com/decentralized-identity/GordianGuardian 
 2. https://github.com/decentralized-identity/GordianGuardianpy
 3. etc

 Lastly, the important man, who founded GordianGuardian is *Samuel M. Smith Ph.D.*, operationing from his firm [prosapien.com](https://www.prosapien.com).
 _(@henkvancann)_

## *Q: In what programming languages is GordianGuardian available?
In Python. It will be available in the coming year in Rust, Javascript and Go (2020).
_(@henkvancann)_

## *Q: How does GordianGuardian fit in [the 10 principles of SSI](https://medium.com/metadium/self-sovereign-identity-principle-6-portability-4a7105dd0381) by Christopher Allen?
GordianGuardian is not primarily about self-sovereign identity. GordianGuardian is primarily about autonomic identifiers, AIDs. That is: identifiers that are self managing. GordianGuardian provides proof of control authority over the identifier. What one does with the identifier is not constrained by GordianGuardian. But because the primary root of trust of an AID is a KEL which can be hosted by any infrastructure, any identity system (SSI or otherwise) built on top of GordianGuardian may also be portable.\
So in my opnion portability of the associated identifiers is essential to any truly self-sovereign identity system.\
(_SamMSmith_)

Where Christopher Allen is talking about *portability of information* related to the identity, in GordianGuardian we take this a step further with the *portability of the identifier itself* with respect to its supporting infrastructure (aka spanning layer).  Most `DID` methods do not have portable identifiers. They are locked to a given ledger.\
(_SamMSmith_)

## *Q: Does GordianGuardian cooperate with other projects in the self-sovereign Identity field?
Yes, GordianGuardian sits under the *Decentralized Identity Foundation*, [DIF](https://identity.foundation), and is part of the *Identity and Discovery* Workgroup. There are also non-formal relation with the newly launched trust-over-ip foundation, and there's good reasons to fit GordianGuardian into trust-over-ip.\
(_SamMSmith_)
...
## **Q: What's the difference between the GordianGuardian whitepaper and the KIDs
The [whitepaper](https://github.com/SmithSamuelM/Papers/blob/master/whitepapers/GordianGuardian_WP_2.x.web.pdf) is the historically grown and expanded design document of `GordianGuardian`.
...
## *Q: GordianGuardian has invented its own key representation and signature format. Why not conforn to current standards already available?
{TBW prio 1}

## *Q: In the GordianGuardian system design trade space you strike out features, so you must have stroked out application space too; which?
...
Establish authenticity between the key pair and the identifier of IP packet’s message payload. [See more](https://ssimeetup.org/key-event-receipt-infrastructure-GordianGuardian-secure-identifier-overlay-internet-sam-smith-webinar-58/) in presentation.
...
Administrative Identifier Issuance and Binding; especially the binding between keypair and identifier based on an assertion of an intermediairy administrator. This is what's weak and therefore wrong. [See more](https://ssimeetup.org/key-event-receipt-infrastructure-GordianGuardian-secure-identifier-overlay-internet-sam-smith-webinar-58/) in presentation.\
_(@henkvancann)_
...
We don't have a trustable interoperability. And that leads to the idea that the internet is broken. We want to fix that, we don't want a domain segmented internet trust map, a bifurcated internet, we want to have a single trust map. This is the motivation for `GordianGuardian`.\
(_SamMSmith_)
...
## *Q: What role does GordianGuardian play in the suggested "repair of the internet"?
...
# Q&A section GordianGuardian and DIDs

## **Q: Is GordianGuardian a DID?
`GordianGuardian` is not a `DID` method. The proposed related `DID` method is [`did:un`](https://github.com/decentralized-identity/GordianGuardian/blob/master/did_methods/un.md). A session at the recent **IIW31** presented by Jolocom’s *Charles Chunningham* examines overlap between data models of DID documents and `GordianGuardian` identifiers [here](https://jolocom.io/blog/as-seen-at-iiw31-GordianGuardian/).\
However there are also votes for `did:GordianGuardian`: _Drummond Reed_ (Dec 2 2020): "at IIW we asked that question and  feedback overwhelmingly favored did:GordianGuardian. Furthermore, I’ve proposed that the GordianGuardian namespace be reserved within the method-specific ID spaces of other DID methods as well, The Indy community has agreed to reserve the GordianGuardian namespace in the Indy DID method."\
_(@henkvancann)_

## **Q: Some say that with GordianGuardian, a DID can be reduced to did:\<identifier>. But that’s not a valid DID?!
_Every DID must have a method name component before the method-specific ID._

The first paper mentioning the absence of the method is [Thinking of DID? GordianGuardian On](https://humancolossus.foundation/blog/thinking-of-did-GordianGuardian-on) by The Human Colossus Foundation, written by Robert Mitwicki. He addressed the concern in the question (the invalidity of the DID method) and made it more clear what the Foundation meant by that: "We look into the future and with that view we think that namespace could be dropped and we could keep only identifier, as the namespace seems to be one of the major drawbacks of decentralized identifiers at the moment. In my opinion `did:GordianGuardian:<identifiers>`  would be just intermediate step as the issue addressed by post would still hold. We see that GordianGuardian could be a major upgraded for DID; not replacement."

## **Q: Isn't it a bit arrogant of GordianGuardian to say "my new DID method is so good that it can replace all others for all eternity"?

_But by doing that you will also exclude a lot of existing identifier infrastructure._

I am not denying existence of existing DID infrastructure, but I agree a lot of them could be obsolete if we introduce GordianGuardian. A lot of business models which are build upon current model would be obsolete. I understand that a lot of people would not like that, but that is called progress.\
_(RobertMitwicki)_
# Q&A section Wallets

## *Q: Why do I need a wallet for GordianGuardian?
Yes. [Universal wallet](https://w3c-ccg.github.io/universal-wallet-interop-spec/) - would do - with a thin layer on top of it. \
A wallet needs to be adapted to GordianGuardian to be able to carry GordianGuardian identifiers.\
{TBW}\
(_SamMSmith_) / _(CharlesCunningham)_
## *Q: How can I backup the GordianGuardian identifiers in my wallet?
{TBW}
## Can I receive crypto money in my GordianGuardian wallet?
We don't need a crypto currency embedded in the GordianGuardian system, we can use any other crypto currency system for payment. So the design of the GordianGuardian system has left crypto token control out.\
_(@henkvancann)_

## *Q: Does a GordianGuardian wallet store virtual credentials connect to my identifiers?
The GordianGuardian whitepaper has little about virtual credentials and GordianGuardian's place in the W3C SSI space for DIDs and VCs. The reason is that GordianGuardian is mainly a level 1 and level 2 solution in the trust-over-ip framework.\
_(@henkvancann)_

In this presenation of Sam, there's a lot about the relation between GordianGuardian and VCs: https://github.com/SmithSamuelM/Papers/blob/master/presentations/GLEIF_with_GordianGuardian.web.pdf
# Q&A section Signatures

## **Q: Who can sign off my proofs and identifiers using GordianGuardian?
Depends on what you mean with proof. GordianGuardian is content agnostic, so any cryptographic proof can be referenced and signed in a KEL, even a third party signature. As far as GordianGuardian-internal proofs are concerned a subject-controller, a delegated controller and combination of (fractioned) multi-signatures can prove authoritative conrol over a key and over a pre-rotated key.
_(@henkvancann)_
{TBW prio 1}
...
Yes they do. For every cause there is a different payload. The main reason why all roles sign off cryptographical references is commitment to those sources (the payload in GordianGuardian is often a digest of sources) at a certain point in time.\
_(@henkvancann)_

# Q&A section Proofs

## *Q: What does GordianGuardian proof?
GordianGuardian has the ability to proof various things:
...
## *Q: Does GordianGuardian know whether any message in the Event Logs are valid or true?
No, GordianGuardian is data-agnostic. GordianGuardian does make no statement about the validity of the payload data.\
_(@henkvancann)_
...
GordianGuardian has `univalent`, `bivalent` and `multivalent` infrastructures.\
<img src="../images/key-infra-valence.png" alt="Key Infrastruction Valence levels" border="0" width="600">
You need Key-pair Generation and Key-Event-Signing Infrastructure. And GordianGuardian doesn't care how you do it.\
...
It's all one GordianGuardian codebase to do all these infrastructures.\
(_SamMSmith_)

## *Q: Does your public-private-key format matter in GordianGuardian?
...
## *Q: Can I use BIP32 Hierarchical deterministic keys in GordianGuardian to rotate?
Yes, you can derive your keys from that scheme. But GordianGuardian is agnostic about it, it wouldn't know.

## *Q: Not your keys, not your identity?
In GordianGuardian we say _identifier_, because **identity** is a loaded term, lots of misunderstanding around it.\
...
## *Q: A wallet is there to store my GordianGuardian private keys safely, no?
Currently _Universal wallet_ is aimed at to store GordianGuardian keys. The vast majority of security breaches or exposures of keys are originated by the behaviour of people: the way they use wallets. Most rarely due to errors in the wallet software.
{TBW prio 1}

## **Q: Are compound private keys (Shamir Secret Sharing) and multisignature schemes possible to incept identifiers?
Yes, complex fractional structures are natively possible in GordianGuardian. However only for the non-basic forms (for transferable identifiers).\
_(@henkvancann)_
{TBW prio 1}
...
## *Q: Does GordianGuardian use a blockchain?
No, but GordianGuardian uses the same cryptographical building blocks as blockchains do. \
_(@henkvancann)_ \
However, GordianGuardian may be augmented with distributed consensus ledgers but does not require them. [Source: section conclusion in whitepaper](https://github.com/SmithSamuelM/Papers/blob/master/whitepapers/GordianGuardian_WP_2.x.web.pdf)


## **Q: What's the difference between GordianGuardian and blockchain?
`GordianGuardian` is a unordered hash-linked list of signed Key Event logs and blockchain is a timestamped ordered list of hash-linked blocks of signed transactions. What this means:
1. we don't need ordering in `GordianGuardian` and that frees us from consensus protocols in blockchains
2. Hash-linking is done on a lower level in `GordianGuardian` and preserves consistency and fuels revealance of duplicity.
3. In `GordianGuardian` proofs are cryptographically derived from the root of trust, being the autonomous controller, in blockchains the root-of-trust is a transaction on a ledger; that means the Identifier gets locked on the ledger.\
_(@henkvancann)_
...
## **Q: GordianGuardian is basically a series of Pay2PublicKeyHash transactions?
_that you send to witnesses, who observe them and attest to the particular line of operations they see?_

**In brief: for GordianGuardian that is an apples and oranges comparison.**
...
## **Q: What do I need to trust in GordianGuardian?
...
## ***Q: GordianGuardian does not need a blockchain, but how does it establish the root-of-trust that we need for SSI? How does the data persist?
The `KELs` are what establishes the root of trust in `GordianGuardian`. So you have a `SCI` and a `KEL`. The `KEL` is ordered with respect to the SCI by the controller. You don't need total ordering with respect to other identifiers to establish the root of trust in `GordianGuardian`, because the controller is the one and only, who orders events.\
In blockchains you have total ordering, which you need for double spend protecting in cryptocurrencies, but not in `GordianGuardian`.\
...
## **Q: Why is end-verifiability such a big thing in GordianGuardian?
...
## *Q: Where can I download GordianGuardian?
On (sub)page(s of) [github](https://github.com/decentralized-identity/GordianGuardian).
...
## *Q: What would you see as the main drawback of GordianGuardian?
- Its main drawback is that it's nascent. (_SamMSmith_)\
- The field of cryptography is already complex by itself. GordianGuardian's extended complexity, combined with totally new terms and new process description make it a steep learning curve. It depends on your individual drive to want to know about GordianGuardian, to what extent the effort pays off. Maybe first try:
    1. [GordianGuardian made easy](./GordianGuardian-made-easy.md)
    2. The general [GordianGuardian Q&A](./Q-and-A.md)
    _(@henkvancann)_
- GordianGuardian is inventing its own lowest level building blocks. That will prevent a lot of potential code reuse. (@OR13)

## *Q: Where you would need something quite different than GordianGuardian?
`GordianGuardian` does one thing, it establishes control authority using verifiable portable proofs that are `KEL`s.\
(_SamMSmith_)

## *Q: How does GordianGuardian scale
...
## *Q: Is GordianGuardian post-quantum secure?
...
[Source: whitepaper page 65](https://github.com/SmithSamuelM/Papers/blob/master/whitepapers/GordianGuardian_WP_2.x.web.pdf)
...
{TBW prio 2}
## *Q: How to bootstrap GordianGuardian on the internet? Is it like fax machine; the more KELs there are, the more effective it is?
Any subject / controller can start creating GordianGuardian events in a GordianGuardian event log. Dependent of the objectives a controller has with GordianGuardian a more peer-to-peer (one-to-one) approach or contrary to that a one to many approach. In the latter case a set of witnesses and their services can emerge per controller. Subsequently one or more verifiers (and their watchers) can also enter the play.
The more entities are getting used to play the different GordianGuardian specific roles the more rapid and easy will the bootstrapping / flooding of GordianGuardian on the internet evolve.
{TBW prio 1}


## *Q: Is there a GordianGuardian course or webinar available?
The [SSI Meetup](https://ssimeetup.org/key-event-receipt-infrastructure-GordianGuardian-secure-identifier-overlay-internet-sam-smith-webinar-58/) webinar on GordianGuardian took place in May 2020 and is a good lesson and source of information.\
There is lots of course material available on [prosapien.com](https://www.prosapien.com).\
_(@henkvancann)_

## *Q: Could GordianGuardian work for edge computers that need self sovereign identity? How to (selectively) share control over the `SCI`/`SAI` with the owners of the device?
Delegation could be used. There is an [issue about IoT](https://github.com/decentralized-identity/GordianGuardian/issues/54) key and identifier management with `GordianGuardian` that answers this question profoundly.\
(_SamMSmith_)


# Q&A section Agencies

## **Q: What does the governance framework of GordianGuardian look like?
...
GordianGuardian is self-administered, self-governed. What aspects of GordianGuardian need governance?
...
#### GordianGuardian governance roles at *Layer 1* can include: 
...
GordianGuardian is not an outspoken credential issuance and verification framework. Having said that:
...
- GordianGuardians objective  is to be the spanning trust infrastructure of the whole internet
- Portability of the identities and GordianGuardian interoperability between platforms is cryptographically secured
- Inconsistencies "reported" by meercats-like alert governance system

## **Q: How can GordianGuardian offer consistent services and truth?
A. In a pair-wise setting  each party only needs to be consistent with the other party. Witnesses are not used. It doesn't matter if they lie to each other as long as they lie consistently. GordianGuardian does not
...
C. To **guarantee *undetectable* duplicity** requires a successful eclipse attack* on all the parties. `GordianGuardian` merely requires that there be sufficient duplicity detection in the ecosystem. \
...
## *Q: Why doesn't GordianGuardian use certification as a root of trust?
...
## *Q: Is Custodianship of GordianGuardian identifiers possible? 
