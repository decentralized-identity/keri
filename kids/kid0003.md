...
sigs = signature offset list (optional all)  
...
Parts:
    ...
    version = major minor = hex characters  10 
...
The lists provide the order that the values from the respective data elements are to be concatenated in the extracted serialized data. When the data element value is itself a collection of data elements such as a list or mapping then the value's elements of the
collection are to be concatenated in the order of appearance of collection's elements.
...
```
But the "keys" element is itself a list. So the concatenated data will be the "sith" value with each key from the "keys" element concatenated in order of appearance in the "keys" list.
