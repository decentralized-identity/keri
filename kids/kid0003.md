...
### Event Receipt Message (Witness(es) or Watcher(s) or Validator(s))

Some web application interfaces might find an enveloped message more convenient. In this case the receipt signatures are embedded in the message itself. This is not as efficient as the streaming format of the version above.
