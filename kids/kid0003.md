...
conf = config data list (icp)   
data = data seal list (rot, itx)  
perm = list of ordered mappings of delegation permissions (dip, drt)  
seal = delegation seal (dip, drt)  
idxs = signature offset list (optional all)  
...
```javascript
{
  ...
  "conf" : [],  // list of config ordered mappings
  "idxs" : []  // optional list of or single lowercase hex string(s) no leading zeros
}
